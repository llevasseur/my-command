...
Parse `--delegate task|god|fb`, `--parallel <n>`, `--sequential`, `--dry-run`, `--mesh`, every flag forwarded to the delegated task workflow — working in place, a base branch, a draft pull request, a subagent clean-and-pull-request stage, and added-command entries — and the goal text.
...
- `--dry-run` — print the routing plan and spawn nothing: no task list, no subagent, no branch. The plan prints each unit's full invocation, its branch and base, the forwarded flags it carries, and the ones dropped for it.
...
- The **task** workflow takes the whole set; nothing is dropped.
- The **god** workflow takes working-in-place, a base branch, and added-command entries; it adds the subagent stage itself, so forwarding that one is redundant rather than dropped. **A draft pull request under `--delegate god` is a stop before planning**, not a per-unit drop: that workflow rejects a draft because a draft cannot merge, so dispatching it would be one stop per unit. Say to delegate to the task workflow instead.
