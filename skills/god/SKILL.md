---
name: god
description: Take a task unattended through implementation, review, CI repair, merge into the chosen merge target, and local update of that branch.
---

# Unattended Task to Merge

Use only when the user explicitly invokes `$god`, `/god`, or requests an unattended task-to-merge workflow. Parse task workspace flags, `--into <branch>`, `--auto`, merge method, `--fix <n>`, and `--no-review`; reject draft mode.

**The cut point and the merge target are two separate decisions.** The workspace
flag that names a base branch is forwarded to `$task` and decides only where the
work branch is cut. `--into <branch>` decides where the finished PR lands, is
never inherited from the cut point, and is the only thing that moves the merge
target: without it the merge target is the repository default branch, so an
invocation that predates the flag behaves unchanged. A run that wants both a
non-default cut point and a non-default landing branch states each one.

1. Record the main checkout and task branch, and resolve the merge target once —
...
2. Re-resolve the PR from GitHub. When its base is not the merge target, retarget
   ...
   untouched, and since `$mc` reads a branch's base from that same PR. Then detect
   conflicts against the merge target locally with
   `git merge-tree --write-tree <merge target> <branch>`; invoke
