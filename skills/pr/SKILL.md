...
3. Use `my-command-tools pr` when available to push and create or update the PR
   ...
   - A branch whose screenshots were taken by a browser gets them published
     ...
     linked from a side branch of itself. A private one gets one attachment
     comment per run instead, since that link would need a credential no
     reviewer's browser has, and the comment's URL comes back with the rest.
     Do not hand-roll that comment: an attached file's URL replaces a body
     reference only where the reference is byte-for-byte the path that was
     attached, and where they differ the images are appended to the end and the
     reference is left broken. Report nothing about screenshots when the helper
     reports nothing: a branch with none, and one verified without a browser,
     both publish nothing on purpose. A warning means something could not be
     published — images with no record beside them, or a comment the CLI
     refused — so name it.
