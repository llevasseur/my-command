...
1. Detect package management, monorepo layout, ignored environment files, generators, repository shell conventions, changelog policy, and the app's boot script, health endpoint, and route directories. Detect Jira **signal** too, and only signal: a Jira board URL in the stack notes, the Atlassian tooling connected in this session, a Jira URL in the README, or issue keys in branch names. The board URL outranks the rest because it is also the answer. None of the four means the repository has no Jira, which is a complete answer, and an existing bootstrap carrying a `jira-contract: declined` marker outranks all of them: the question was asked and answered no, so ask nothing however loud the signal. There is no board-URL flag and there must not be one; the notes are the fast path.
2. Ask one focused round only for details that cannot be discovered. Then ask a second round for the run contract: boot command, health probe URL, seeded dev login, and a map of source globs to routes. State when asking that the login is used only against a localhost URL the verification run itself booted. A repository with no app to boot records that and emits no contract; never invent a boot command to fill the field.
   - **Ask a third round for the Jira contract only where step 1 saw signal**, and never ask a repository whether it uses Jira when nothing in it says so. **Open that round with a yes/no, because signal is not consent**: signal says a Jira exists near this repository, not that its tickets belong there. On a no, write a `jira-contract: declined` comment where the contract would have gone — the spot the `--print-jira-contract` branch would have occupied — emit no such flag, and stop. That marker is read by the next run of this command, which then asks nothing; without it a stale issue key in an old branch re-opens the interview forever. Deleting the line is how someone re-opens it, and this command never deletes it for them.
   - On a yes, **query Jira for the facts rather than asking them to be recited**. **Parse a board URL from the notes first**, since it answers site, project key, and board id at once. Handle the team-managed `https://<site>/jira/software/projects/<KEY>/boards/<ID>`, the company-managed form with the extra `/c/` segment, either with a trailing `/backlog`, and the legacy `https://<site>/secure/RapidBoard.jspa?rapidView=<ID>&projectKey=<KEY>` where `rapidView` is the board id. Confirm the parsed site and key against the reachable sites and visible projects rather than trusting them — a key that site does not have is a URL pasted from the wrong Jira, and saying so beats a contract that resolves to nothing at run time. Then read the project's issue types for their names and ids, and read the live transitions from one real issue in the project. Record for `start` and for `review` the transition id, the transition name, **and** the target status's id and name — all four, because `$ticket` checks the declared name and id against the live workflow before firing and confirms the status afterwards. Ask which transitions are forbidden and why, then the board, the sprint, the default issue type, and the blocking link type, and map each issue type to a default template.
   - **The board is three paths tried in order, and only the last asks.** A URL parsed from the notes already carries the id. Otherwise look the boards up when the API token is there: the Atlassian tooling exposes no board or sprint surface, but the Agile REST API does, reached with the same auth `$ticket` uses for attachments — an API token from the macOS Keychain under service `my-command-jira`, the account being the email resolved from whoever is authenticated, piped into `curl --config -` so it never reaches argv. Probe first and treat a non-zero exit as the absent case rather than an error; `GET /rest/agile/1.0/board?projectKeyOrId=<KEY>` returns the project's boards under `values`, each with an id and a name. Offer them **by name** as a structured choice, since the name is the only part anyone recognizes; take a lone board without asking and say which one was taken. With no URL and no token, ask in prose rather than as a structured choice — a URL is free text, not a discrete option — for the board's **URL**, whatever the address bar shows on the board or its backlog, never "the board id", which nobody knows. Offer skipping Jira for this repository in the same breath; a skip there takes the decline path, marker and all.
   - **The sprint policy is the one answer with no URL behind it**, so ask it outright — null for no sprint, a specific sprint id, or `active` for whichever sprint is open when a ticket is created — and **offer `active` as the default**, since it is what a team working off a board almost always means and the only one of the three that cannot go stale. With a board id in hand it is checkable rather than taken on trust: `GET /rest/agile/1.0/board/<ID>/sprint` through the same authenticated call lists that board's sprints, each carrying a state of future, active, or closed.
   - **Never write a cloud id or an Atlassian account email into the contract**: the cloud id is resolved at run time from the site, so a site migration cannot leave a pinned id pointing at the wrong tenant, and the account email is resolved from whoever is authenticated, because the contract is committed and shared while that email belongs to the person running the command. This leg is **additive and re-runnable** — a repository that already has a bootstrap gains only the Jira contract, and an existing Jira contract is updated in place. A repository carrying the declined marker is the one case never re-asked.
3. Create or update tracked `scripts/bootstrap-worktree.sh` and optional `AGENTS.md`/`CLAUDE.md` guidance. Make it portable: discover the main checkout via git common-dir, refuse to run there, link only ignored existing environment files without overwriting, install in the worktree, and regenerate from worktree-owned inputs. Give it a `--print-verify-contract` flag that prints `{boot, health, login, routes}` as JSON on stdout and exits, handled ahead of the main-checkout refusal so a caller with no worktree can still read it. Omitting the flag is allowed: a verification run then falls through to detection, reading a dev, start, or preview script and taking the real bound port from the startup log. Where the third round produced a Jira contract, give it a `--print-jira-contract` flag on the same terms, handled alongside the first one and ahead of the refusal, printing the site, project key, board, sprint, default issue type, issue types, lifecycle, forbidden transitions, and link type — and no cloud id. A repository with no Jira, and one that declined, omits the flag and `$ticket` skips; where the answer was a decline, the `jira-contract: declined` comment goes in the flag's place.
