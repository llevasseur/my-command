...
**One exception to untouched: a `ticket` entry is rewritten to invoke `/my-command:ticket --yes`.** It is a guarantee rather than a fix for a live prompt, and the difference is worth stating so nobody later "simplifies" it away on the grounds that nothing currently prompts. An `--add` entry reaches `/my-command:ticket` through `/my-command:task`, where it runs **adopt-only** — it never creates, so the creation approval `--yes` exists to skip is not on that path today. The flag is appended anyway because this command never asks a question, and the cost of being wrong is asymmetric: a forgotten flag costs an unattended run that hangs on a prompt nobody is there to answer, while a redundant one costs nothing. It does **not** reach `/my-command:ticket`'s `never` list, which refuses a forbidden transition whatever flags it is handed.
