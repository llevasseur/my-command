...
**One exception to untouched: a `ticket` entry is rewritten to invoke `/my-command:ticket --yes`**, so an unattended run never hangs on a prompt. It does **not** reach `/my-command:ticket`'s `never` list, which refuses a forbidden transition whatever flags it is handed.
