...
- **A browser-verified branch shows its screenshots on the PR, whether or not the repo is public.** The verb finds the screenshots `/my-command:verify` captured for this branch, and publishes them when the `verdict.json` recorded beside them says a **browser** tier took them: a `## Screenshots` section where every image sits in a markdown table cell, even a lone one, as a bold label, the image, then one sentence on what it proves; before/after pairs share a row per view, the rest fill a two-column grid, and the comment closes with "What these shots do not prove". It reports `screenshots` with the count, tier, verdict, and `via`. **The gate is the recorded tier, not the diff or the verdict.** The labels, sentences, and gaps are the verifier's own read-back as `shots record --shot` and `--gap` stored them, never written here: a shot **no** record ever described is published as unlabelled and named in `shotsWarning`, and the fix is to record the verifier's `saw:` line for it, not to invent one. A branch verified more than once has a verdict file per run, and they are merged — an earlier run's label still describes its own shots, so unlabelled means nobody ever read that image back. The caption says the images were *captured and inspected*, which is what the browser tier means.
  - **`via: "comment"`** is the only route, and every repository takes it whatever its visibility: one `gh pr comment --attach` per PR, reused when the images are unchanged, with its URL reported as `comment`. Never post screenshots yourself with `gh pr comment --attach`; the verb handles the path matching.
