...
7. If this session is running in a git worktree, remove it now — the branch is already pushed to origin, so removing the local worktree keeps the branch checkout-able later without losing work:
   ...
   - If in a worktree, call the `ExitWorktree` tool with `action: "remove"`. If it reports unpushed/uncommitted commits would be discarded, confirm the commits are already on origin (they were pushed in step 2), then re-invoke with `discard_changes: true` — this only discards the local worktree copy, not the remote branch.
