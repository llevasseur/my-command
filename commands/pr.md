...
- **A browser-verified branch carries its screenshots into the body.** The verb finds the screenshots `/my-command:verify` captured for this branch, and publishes them when the `verdict.json` recorded beside them says a **browser** tier took them: a `## Screenshots` section, before/after pairs as a table with one row per view, any unpaired group as a grid. It reports `screenshots` with the count, tier, and verdict. **The gate is the recorded tier, not the diff** — a backend change proven through a frontend that needed no edit still shows its screenshots, and a `red` verdict shows them too, because those are the ones a reviewer most needs. **Say nothing about screenshots when it reports nothing**: a branch with none, and one verified at a non-browser tier, both attach nothing by design. A `shotsWarning` means it had something to attach and could not — usually screenshots with no recorded verdict, which is `/my-command:verify` Step 6 having been skipped — so name it in the report.
