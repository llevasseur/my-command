...
- **A browser-verified branch shows its screenshots on the PR, whether or not the repo is public.** The verb finds the screenshots `/my-command:verify` captured for this branch, and publishes them when the `verdict.json` recorded beside them says a **browser** tier took them: a `## Screenshots` section, before/after pairs as a table with one row per view, any unpaired group as a grid. It reports `screenshots` with the count, tier, verdict, and `via`. **The gate is the recorded tier, not the diff** — a backend change proven through a frontend that needed no edit still shows its screenshots, and a `red` verdict shows them too, because those are the ones a reviewer most needs.
  - **`via: "body"` is the public-repo route.** The bytes go on the repo's `my-command-shots` branch and the body links `raw.githubusercontent.com`, reported as `ref` and `commit`.
  - **`via: "comment"` is what a private repo takes**, because that raw link needs a credential no reviewer's browser has. The verb posts one `gh pr comment --attach` per run — never one per image, and at most 50 files in the one call — which uploads each file to GitHub's `user-attachments` CDN and renders under the reader's own credential. The comment's URL comes back as `comment`. **The reference-matching rule is why the body reads as a table and not a pile of images**: `gh` rewrites a `![alt](<path>)` reference to its uploaded URL only where that path string is byte-for-byte the one `--attach` was given, and where they differ it silently appends every image to the end and leaves the reference broken. The verb writes both sides from the same absolute path, so this is handled — but never hand-roll a `gh pr comment --attach` alongside it with a relative path in the body.
  - **Say nothing about screenshots when it reports nothing**: a branch with none, and one verified at a non-browser tier, both publish nothing by design. A `shotsWarning` means something could not be published — screenshots with no recorded verdict, which is `/my-command:verify` Step 6 having been skipped, or a comment `gh` refused — so name it in the report.
