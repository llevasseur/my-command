...
- **A frontend change carries its screenshots into the body.** When the branch's diff touches frontend code — markup, stylesheets, or a script file under a directory that serves a UI — the verb finds the screenshots `/my-command:verify` captured for this branch, publishes them, and appends a `## Screenshots` section: before/after pairs as a table with one row per view, and any group with no pairing as a grid. It reports `screenshots` with the count. **Say nothing about screenshots when it reports nothing** — a diff that changes no frontend code, and a frontend change with no screenshots, both attach nothing by design. A `shotsWarning` means it had something to attach and could not, so name it in the report.
