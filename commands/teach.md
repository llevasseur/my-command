---
description: Learn the real terminology for a technique you can only describe, grill the plan into a spec, then copy a /my-command:god invocation written in that vocabulary
...
- `--to <command>` — target command for the hand-off. Default `god`. Passed to `/my-command:cp` as its first token.
...
## Step 4 — Hand off to `/my-command:cp`

One `Skill` call: `skill: "cp"`, `args:` the target command (default `god`) followed by the criteria.

Write the criteria in the learned vocabulary — the term from Step 1, the decisions from Step 2, and the constraints the user stated. It has to stand alone for an agent that never saw this conversation. Don't pass `--verbatim`; `/my-command:cp` shaping on top of it is fine.

`/my-command:cp` owns the clipboard write and never runs the target. **Never invoke `/my-command:god` (or whatever `--to` names) yourself, and never print the composed criteria** — printing it is the cost `/my-command:cp` exists to avoid.
...
- Never read files, grep, or search in Step 4 — that research belongs in Step 1, and `/my-command:cp` does none of its own.
