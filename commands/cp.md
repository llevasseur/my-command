...
1. **Restore, with `--again` / `-a`.** This flag short-circuits the whole command: do not compose, do not read anything, do not enrich anything, and ignore any command or prompt tokens rather than acting on them. One Bash call, which spends no tokens on the text it restores:
   ...
   A slot number reaches an older ring entry — `/my-command:cp --again 2` is `my-command-tools stash restore 2`, valid through slot 4. The verb detects the clipboard sink itself, so there is nothing to substitute per platform. If the slot holds nothing it says so and leaves the clipboard alone — an empty clipboard is worse than whatever is on it now — so repeat that plainly and write nothing — that run has no slot to name, because the verb reported a failure instead of an entry. Then stop — steps 2 and 3 do not run. A restore that *succeeded* names the slot it read; carry that number into the reply the way a compose run does.
...
3. **Copy.** Two calls, in this order — the stash and the clipboard end up carrying identical bytes, so a later copy from anywhere else can be undone with step 1. First write the composed line with the `Write` tool, spelling the home directory out — the tool does not expand `~`:
   ...
   **`--consume` is not optional.** The compose file is a hand-off, not a document: once the verb has copied its bytes into the ring it deletes it. A unique name is what makes the write succeed; deleting it is what keeps one file per `/my-command:cp` from piling up under `~/.claude` forever.

   **Read the slot out of the result — it is what the reply carries.** The call above prints one line — `stashed N bytes in slot 0` — and that line is where the number comes from. The same slot is a field on the `--json` payload, which this step has no reason to ask for. A compose always lands in the newest slot, so the number is always 0; it is read rather than assumed because a reply naming a slot it guessed is worse than one naming none.

   **The entry travels as a file path, never as a shell-composed string.** `cat > … <<'EOF'` composes a file in the shell, and that shape is refused outright inside an isolated worktree — which is where `/my-command:cp` is often invoked from — while an argument would need every quote, backslash, and newline in the prompt escaped correctly. `Write` takes the content literally and the verb copies the bytes, so nothing expands or escapes either way.
...
- The whole reply is `Done!` followed by the slot the copy is in — `Done! 0` — then at most one short line naming the direction taken (what you resolved, assumed, or preserved verbatim). Nothing else — no preamble, no composed command, no next steps. The slot is what reaches that copy again without recomposing it: `/my-command:cp --again 0` puts that entry back, and every later copy shifts it one slot older. Take the number from the verb's own report on both paths — a compose names the slot it wrote, a `--again` run the slot it read — never from a guess. The one reply that carries no slot is a `--again` against an empty slot: nothing was copied, so there is no entry to name and the reply is that failure alone. <!-- include: shared/text-only-turn.md -->Deliver that report in this run's **closing turn** — the terminal step below — rather than alongside the tool call that precedes it.<!-- /include -->
