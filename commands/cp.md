...
3. **Copy.** Two calls, in this order — the stash and the clipboard end up carrying identical bytes, so a later copy from anywhere else can be undone with step 1. First write the composed line with the `Write` tool, spelling the home directory out — the tool does not expand `~`:
   ...
   **Read the slot out of the result — it is what the reply carries.** The verb reports a `slot` alongside `path` and `bytes`, and its one-line status says `stashed N bytes in slot 0`. A compose always lands in the newest slot, so the number is always 0; it is read rather than assumed because a reply naming a slot it guessed is worse than one naming none.
