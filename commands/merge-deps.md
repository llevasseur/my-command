...
description: >
  ...
  /my-command:mc -t <branch> to merge main in and resolve conflicts, verify in an isolated worktree,
...
`/my-command:mc` it up to date with `main`, verify it green in an isolated worktree, then merge
...
2. **Verify in an isolated worktree.** After `/my-command:mc`, `B` is up to date with `main` and
   checked out nowhere — `/my-command:mc -t` does its merge in its own worktree and tears that worktree
   ...
   - `my-command-tools worktree begin --branch B --existing --bootstrap`. `--existing`
     checks `B` out rather than creating it — creating it would throw away the bump `/my-command:mc`
...
5. **Refresh local main** before the next PR so its `/my-command:mc` resolves against the just-merged
   result: `git checkout main && git pull --ff-only origin main`. (Skip the pull effect for
   queued/auto PRs whose merge hasn't landed yet — `/my-command:mc` fetches main itself regardless.)
...
3. Report a concise summary:
   - ✅ merged into `main`
   - 🟡 had conflicts `/my-command:mc` resolved, then merged (name the files `/my-command:mc` touched)
...
- Delegate all conflict resolution to `/my-command:mc` — never hand-merge here. Never force-push;
  merges only.
- Never merge a PR whose `/my-command:mc` conflicts are unresolved or whose verification failed.
