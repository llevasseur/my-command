---
description: Lease a skill for one task and give it back, counting the install on the concept record /my-command:teach already writes
argument-hint: "[--field|-f <field>] [--concept|-c <term>] [--skill|-s <pkg@skill>] [--keep|-k] [--dry-run|-n] <the task you are about to do>"
allowed-tools: Bash, Read, Skill
---

`/my-command:teach` points at a public skill and stops — Step 5 surfaces the names and is forbidden from installing any of them. So the last mile is done by hand every time: find the skill again, install it, use it, and then either remember to remove it or let it accumulate. This command is that last mile, and it takes the install as a **lease**: the skill arrives for one task and goes back on the way out.
...
- `--field` / `-f <field>` — the field the concept belongs to (`UI motion`, `domain modeling`, a business area). Passed to `/my-command:lookup` to scope the neighbours, and used to scope the skill search when the corpus surfaces nothing.
...
**Run `/my-command:lookup` on the query, with `--field` set when `--field` was given.** The task text is the query unless `--concept` named one. This is the first step because the concept is what the install is counted against, and because the corpus is where the candidate skills come from.
...
- **A miss** — the corpus holds nothing. Say so, and name `/my-command:teach <the description>` as the way to create the record a future lease would count on.
...
**Never re-word a stored sentence, and never name a term `/my-command:lookup` did not return.** This command does not teach; a run that invents a term to have something to count against has manufactured the record it was supposed to read.
...
1. **The stored record's `surfacedSkills`.** These are the skills `/my-command:teach` **discovered** and pointedly did not install — the list that exists because those names used to be dropped on the floor. A lease is the reader that list never had, so read it before searching for anything.
2. **The stored record's `skills`.** The skills a `/my-command:teach` run applied to name the concept. A skill good enough to name the thing is usually good enough to work on it.
...
## Step 4 — Count the install on the record `/my-command:teach` already writes
...
```bash
my-command-tools concepts count "<the stored term, exactly as /my-command:lookup returned it>" "<the leased skill name>"
```
...
**This command still writes no code of its own and opens no PR.** For work that ends in a PR, compose it into the run that does: `/my-command:task -a learn <how the skill relates> <the task>` puts the lease around the implementation, so the skill is present while the work happens and goes back when that run ends.
...
Then report, in two or three lines: the concept and the outcome `/my-command:lookup` reached, the skill leased and whether it was returned or kept, and whether the install was counted or why it was not.
