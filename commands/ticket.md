...
**`/my-command:manage` calls this verb once a wave has landed**, one call per edge of its dependency graph
whose two units both carry an issue key; the rule is stated in `/my-command:manage`.
...
- **It attaches the PR to the item as a remote link once the PR is open** — directly, with the
  ...
  `/my-command:ticket <KEY>` default once a PR is open**, because that default would fire `review`.
- **It never fires the `review` transition.** Several `/my-command:task` runs can feed one ticket, so
  `review` stays an explicit `/my-command:ticket move <KEY> review`.
