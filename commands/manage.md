...
- `--add <list>` / `-a <list>` — forwarded to **every** delegated run, in the same comma-separated `<command> <prompt>` shape `/my-command:god` forwards it in. It reaches each unit's `/my-command:task`, not this command.
...
```
Run /my-command:task --add <forwarded list> <this unit's criteria>.
