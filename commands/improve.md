...
Improve the agentic workflow using evidence instead of intuition. claude-proxy reads every ten recorded sessions and reports what would have reached the same outcome in fewer steps. This command collects those findings, turns the pending ones into task criteria, runs `/my-command:task` on them in a subagent per target repo, and records which suggestions were actually applied so the next run doesn't re-propose them.
...
- Anything not listed above that `/my-command:task` recognizes is **passed straight through** to every `/my-command:task` invocation in Step 4 — currently `--here` / `-h`, `--base <branch>`, `--draft` / `-d`, and `--add` / `-a <list>`. Read `/my-command:task`'s own Flags section rather than duplicating its list here; this command does not interpret them itself.
...
- **If `CLAUDE_PROXY_STORE` is unset, or its path is missing, or the derived checkout has no `server/package.json`, stop.** Say which of the three failed, that `/my-command:improve` has no suggestions to read without it, and that it must be exported in the shell environment — e.g. in `~/.zshrc`:
...
Suggestions carry a status flag: `pending` by default, `done` once applied, `skipped` when deliberately passed over. **Read only the pending ones** — that is what keeps a later `/my-command:improve` over the same range from re-proposing work that already shipped.
...
Turn the pending rows into criteria a `/my-command:task` run can implement without going back to the proxy.
...
**Group the criteria by the repo they land in**, then dispatch **one fresh subagent per repo** via the `Agent` tool, each running `/my-command:task` with the composed criteria for that repo and the pass-through flags exactly as given:

```
/my-command:task <pass-through flags> <composed criteria for that repo>
```
