---
description: Turn claude-proxy's session suggestions into an implemented improvement — read what the agent keeps doing the slow way, hand it to /my-command:task as criteria, and flag the suggestions that shipped as done
argument-hint: "[--range|-r <spec>] [--dry-run|-n] [--here|-h] [--base <branch>] [--draft|-d] [--add|-a <list>] [extra context]"
---

Improve the agentic workflow using evidence instead of intuition. claude-proxy reads every ten recorded sessions and reports what would have reached the same outcome in fewer steps. This command collects those findings, turns the pending ones into task criteria, runs `/my-command:task` on them in a subagent, and records which suggestions were actually applied so the next run doesn't re-propose them.
...
- Anything not listed above that `/my-command:task` recognizes is **passed straight through** to the `/my-command:task` invocation in Step 4 — currently `--here` / `-h`, `--base <branch>`, `--draft` / `-d`, and `--add` / `-a <list>`. Read `/my-command:task`'s own Flags section rather than duplicating its list here; this command does not interpret them itself.
...
Dispatch **one fresh subagent** via the `Agent` tool to run `/my-command:task` with the composed criteria and the pass-through flags exactly as given:

```
/my-command:task <pass-through flags> <composed criteria>
```
