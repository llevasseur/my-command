...
3. If the review produced an `/my-command:fb` line: run it **inline in this session** via the `Skill` tool (`skill: "fb"`, `args:` the feedback text after `/my-command:fb`). Never dispatch `/my-command:fb` to a subagent via the `Agent` tool — the independence a subagent buys is the *reviewer's*, and that was already spent in Step 3; applying the findings is ordinary work on this branch, and running it here keeps the findings you just read in context. This runs inside the same worktree/checkout from Step 2, so `/my-command:fb`'s default (current branch, no `--target`) is correct — do not add `--target` yourself.
