...
- `--worktree <path>` — the workspace already exists; whoever invoked this run made it. Work through absolute paths under `<path>` and create nothing, enter nothing, and tear nothing down. This is set by a dispatching command rather than typed by hand. It wins over `--base`, since the branch is already cut, and `--here` wins over it, since `--here` names the current checkout outright.
...
**`worktree begin` reports a `path`, and that path is this run's working root.** Resolve every read, edit, commit, and `--cwd` under it, as an absolute path. That is the normal way this command works — not a fallback after something was refused — and it is the only way that works for a run dispatched as a subagent or started in a different repo than the one being changed. Tear down with `my-command-tools worktree end`, which re-verifies the branch reached origin before removing it.

`EnterWorktree` moves the *session* into that path. It is optional, it is available to exactly one kind of run — one **you** invoked directly, in the repo this session started in — and it changes nothing about the paths above, which stay correct either way. If the repo being changed is not the repo this session started in, prefer starting a new session in the target repo.
...
- **`--base <branch>` given:** add `--base <branch>` to the same `worktree begin` call, then work under its `path` as above.
- **`--worktree <path>` given:** the workspace is already there and is not this run's to manage. Do not call `worktree begin`, do not call `EnterWorktree`, and skip Step 1.5 — the dispatcher bootstrapped it. Read `my-command-tools state --cwd <path>` to learn the branch you are on, resolve every read, edit, and commit as an absolute path under `<path>`, and pass `--cwd <path>` to every toolkit verb that takes it. Step 3's teardown is skipped for the same reason: the caller owns the path and removes it after collecting this run.
...
**Skip this entirely for `--here`** (the current checkout is already bootstrapped) **and for `--worktree <path>`** (the dispatcher bootstrapped it when it made the path, and re-running an install underneath a workspace someone else owns is how two concurrent units end up fighting over one lockfile). A fresh worktree has no `node_modules`, no `.env` files, and no generated code — without them `tsc`/`biome`/tests silently under-check or fail outright, so do this **before** implementing.
...
- **Verify before claiming done** — `my-command-tools verify`. It discovers the repo's own gates and runs them fastest-failing first, returning no log for a gate that passed and a bounded tail for one that failed. Narrow it with `--only <script,...>` when only part of the repo was touched. Report which gates ran; `pass: false` means you are not done. A full sweep outlives the Bash tool's window, so start it detached and **block on it with one call** — the section below this list is how.
...
4. **Fix what it reports on this run's own changes.** Code, structure, and naming are all in scope here, unlike in `/my-command:clean`. A finding on a line this run never touched is pre-existing: leave it and name it in the report rather than widening the diff into unrelated cleanup.
...
verdict — because that record is what lets Step 3's `/my-command:pr` publish the screenshots: it does so
