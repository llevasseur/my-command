...
- **Every exit routes here, not just the shipped one.** PR opened; no-change run; `verify` still failing; a step blocked, refused, or awaiting my answer; criteria abandoned as wrong. The wording changes, the closing turn does not. A run that stops early still gets one — say where it stopped, what is on the branch, and leave `/my-command:revive <thread id>` as the recovery path when the proxy thread id is available.
...
- **`--sub` does not move this step.** The subagent runs `/my-command:clean` + `/my-command:pr` and reports back into this session; its report is not this turn. After the `Agent` call returns and teardown finishes, close the run here, in this session, in a message of your own.
