...
3. **That diff is the comment context — never re-fetch it a file at a time.** Every added and modified line this command may touch is already in step 2's output, hunks and all, so the changed-file list is not something to walk. Walking it is the named failure this step replaces: a scope call returning eighteen paths, then eighteen single-`Read` turns before the first edit, no one of which depended on any other. `files` is the enumeration the batched-discovery step above asks for — hand every path to a single `git diff <diffRef> -- <path> <path> …` rather than one call per path, and read the comments off the hunks that come back.
