...
- `--yes` / `-y` — skip the creation approval **and** the template question together. The
  template is inferred from the diff instead of asked for. `/god` passes this, so an unattended
...
**Discovery mirrors `/task` Step 1.5** — the repo states its own facts, and this command reads
them. Take the first source that answers:

1. **`bash scripts/bootstrap-worktree.sh --print-jira-contract`** — parse its stdout as JSON.
   The script is the same one `/task` Step 1.5 and `/verify` read, and the flag is handled ahead
...
3. **Neither exists → this repo has no Jira.** Warn once, **skip all ticket work**, record the
   skip in this run's report (and in the PR description when composed into `/task`), and say that
   `/task-bootstrap` is what writes the contract. **Do not invoke `/task-bootstrap`** — adding a
...
or the PR in place of reading it. A composed `/task` run keeps going and reports the gap; a
standalone `/ticket` run reports it and stops there.
...
**Add a fourth, `Testing Done`, only when the run actually exercised something** — a `/verify`
...
3. **On any mismatch — id gone, name changed, or the two now naming different transitions — stop
   ...
   the contract needs regenerating with `/task-bootstrap`. Do not fire the closest match: a
...
**`/manage` passes the dependency graph it already builds for its waves.** That graph is the one
...
it first — `security find-generic-password -s my-command-jira -a <email> -w >/dev/null 2>&1` —
...
## Composing into `/task` with `--add`

`/task -a ticket <prompt>` weaves this command into a task run. In that position it is
