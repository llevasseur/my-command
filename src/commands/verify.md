...
publishable: `/pr` publishes a branch's screenshots when this record says a **browser** tier
...
verbatim, and one `--gap` per `gap:` line.** `/pr` publishes those words under each image and
...
- Persisted Playwright specs are out of scope: the specs and logs the verifier writes are
  ...
  `/pr` publishes them into the PR body when Step 6's record says a browser took them. That is
