...
3. If the review produced an `/fb` line: run it **inline in this session** via the `Skill` tool (`skill: "fb"`, `args:` the feedback text after `/fb`). Never dispatch `/fb` to a subagent via the `Agent` tool — the independence a subagent buys is the *reviewer's*, and Step 3 already spent it; running `/fb` here keeps the findings you just read in context. This runs inside the same worktree/checkout from Step 2, so `/fb`'s default (current branch, no `--target`) is correct — do not add `--target` yourself.
...
- This command doesn't post the review as a GitHub PR comment/review (`gh pr review`) — its output is the `/fb`-ready text, both shown to me and applied locally.
