...
- Free text — focus terms. `/health node` ranks the whole machine but expands the node owners; `/health chrome slack` expands both. Focus never hides a heavier owner, because "what is actually eating the machine" is the question even when the user asked about one process.
