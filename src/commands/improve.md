...
**Group the criteria by the repo they land in**, then dispatch **one fresh subagent per repo**, each running `/task` with that repo's composed criteria and the same pass-through flags exactly as given:
