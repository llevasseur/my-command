...
- Delegating to `/task` means `/task`'s rules apply — it has standing permission to commit on the branch (never on `main`), and it ends at a PR. `--yes` / `-y` governs the size-guard confirmations, not whether a PR gets opened.
