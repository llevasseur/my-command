...
**One exception to untouched: a `ticket` entry is rewritten to invoke `/ticket --yes`.** It is a guarantee rather than a fix for a live prompt, and the difference is worth stating so nobody later "simplifies" it away on the grounds that nothing currently prompts. An `--add` entry reaches `/ticket` through `/task`, where it runs **adopt-only** — it never creates, so the creation approval `--yes` exists to skip is not on that path today. The flag is appended anyway because this command never asks a question, and the cost of being wrong is asymmetric: a forgotten flag costs an unattended run that hangs on a prompt nobody is there to answer, while a redundant one costs nothing. It does **not** reach `/ticket`'s `never` list, which refuses a forbidden transition whatever flags it is handed.
...
**Point the PR at the merge target first.** Step 3 already read `baseRefName`; when it is not the merge target, retarget the PR with `gh pr edit <number> --base <merge target>` in its own call before anything below. `/pr` opens every PR against the default branch by design and is left untouched as the PR wrapper, so under `--into` this is the step that makes the PR's base true. It has to run before `/mc`, which resolves a branch's base from that same open PR. A PR already based on the merge target is left alone.

The merge target may have moved while `/task` worked. Test **locally** — `git fetch origin`, then `git merge-tree --write-tree <merge target> <branch>`; GitHub's `mergeable` is lazy and reports `UNKNOWN` for a fresh branch.
...
In the **main checkout** recorded in Step 1 (not a worktree — `/task` removed the one it made), acting on the merge target resolved in Step 1 — the default branch, or `<branch>` under `--into`:
