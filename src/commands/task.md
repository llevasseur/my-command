...
**First, confirm this run actually produced changes — if it did not, skip this stage.** `/clean` and `/pr` only make sense when there is something to ship; on an empty diff they're wasted work and `/pr` would push a branch and open a PR with no content. Check both halves of the state:
