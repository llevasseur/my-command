...
- `--add` / `-a` — register one or more commands available to the user for the agent to weave into this `/task` run, each paired with a prompt that guides its use. See "Added commands" below.
...
`--add` lets me extend a `/task` run with commands available to the user without changing this file. Each added command carries an associated prompt — the free text after the command in its entry — that the agent uses to decide whether and where it fits in the flow above.
