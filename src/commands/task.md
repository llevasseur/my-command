...
1. **No run contract and nothing detectable.** `bash scripts/bootstrap-worktree.sh
   ...
   without an app has not opted in, and opting it in is `/task-bootstrap`, not a side effect
   here.
...
description; nothing here blocks a merge, and `/god`, `/manage` and `/dev` are unchanged — a red
...
- **Record the outcome accurately.** Completion means the PR exists and any dedicated worktree is removed. A failed or interrupted run is not complete; say where it stopped and leave `/revive <thread id>` as the recovery path when the proxy thread id is available.
