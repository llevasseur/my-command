...
**Anchor Step 4 before the first tool call.** Put this run's steps in the harness's todo/task list, and make **Step 4's closing turn the last item** — worded as its own item ("close the run in a text-only turn"), never folded into "open the PR" or "tear down the worktree". Leave it open until it is the only item left. Do this now, before `state`, before the worktree, before anything: the todo list is live session state that a compaction carries forward, and this prompt is not. Once the run is summarized, that item is the only surviving record that a closing turn is owed.
...
**This step is never skipped and never delegated.** The run is not over when the PR opens or the worktree is removed — it is over when this session sends **one final message that contains text and no tool call**. That is the mechanic, not a style preference: the outcome of a run is recorded from a message with no tool call in it, so a message carrying the report *and* a tool call is recorded as a decision mid-run, and a run whose last message is a tool call is recorded as having no outcome at all. Make the tool call, let it return, then reply with text alone.

- **Every exit routes here, not just the shipped one.** PR opened; no-change run; `verify` still failing; a step blocked, refused, or awaiting my answer; criteria abandoned as wrong. The wording changes, the closing turn does not. A run that stops early still gets one — say where it stopped, what is on the branch, and leave `/revive <thread id>` as the recovery path when the proxy thread id is available.
- **Say it in one self-contained line first**, then any detail: what shipped, the branch, the PR number/URL — or what stopped the run. Someone who never saw the criteria should be able to read that line alone.
- **`--sub` does not move this step.** The subagent runs `/clean` + `/pr` and reports back into this session; its report is not this turn. After the `Agent` call returns and teardown finishes, close the run here, in this session, in a message of your own.
- **If context was compacted mid-run**, the Step 1 todo item is the reminder that this is still owed. If the todo list went with it, close the run anyway: any run that reached this far owes a closing turn, and sending a redundant one costs nothing while skipping it loses the record.
- Do not tack the report onto the same message as `ExitWorktree`, `worktree end`, `verify`, or a `gh` call — those are exactly the calls that sit at the end of a run and swallow the outcome.
