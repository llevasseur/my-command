...
The ledger is an **append-only event log** on the existing `operator` Cloudflare Worker over D1, reached with **`IDEAS_URL`** and **`IDEAS_TOKEN`** (with `CONCEPTS_URL` / `CONCEPTS_TOKEN` accepted as documented fallbacks, since ideas and concepts sit behind one Worker).

Both calls this command makes go through a **store hook** in `~/.claude/my-command/hooks/`, installed beside the workflow gates and allowlisted by name so neither costs an approval round-trip: `ideas-read.mjs` for the dedupe read, and `ideas-add.mjs <path-to-json>` for the write. The write takes a **path** rather than an inline array because a rationale carries newline bullets, and composing that on a command line is the heredoc shape this repo's gates refuse — the proposals are written to a scratch JSON file with `Write` and the path is passed. Each hook prints one status line and always exits 0, so the run reads the line rather than the exit code, and the token is read from `process.env` inside the hook's own process rather than being passed as an argument.
 It used to be `<logDir>/ideas.json`, a device-local file beside the proxy's logs, behind a three-tier waterfall that fell through to the repo's own `docs/ideas.md` and then to `~/.claude/ideas/<repo-slug>.md`. **All three tiers are gone**, and `pnpm --filter server ideas` is now a client of the hosted store rather than the owner of a file — so its verbs take no `LOG_DIR`, though the `suggestions` verbs still do.
