...
- **Command toolkit** — a zero-dependency Node CLI (`my-command-tools`) that owns the deterministic half of a workflow run, so commands stop re-deriving the same git state with a slightly different volley of one-off shell calls each time. Seven verbs, each returning **JSON on stdout**: `state` (branch, base, commits, tracked vs untracked changes, and a single `hasWork` boolean that settles `/task`'s no-change gate), `verify` (discovers the repo's package manager by lockfile, runs its gates fastest-failing first, and returns *no* log for a pass and a bounded tail for a failure — no more hand-rolled `2>&1 | tail -12`, and no re-running a whole build because the window was guessed too small), `commit`, `pr`, `worktree begin|end|list`, `clean-scope`, and `doctor`. The guards are the point: `commit` refuses the default branch and refuses `.`/`-A`-style whole-tree staging so carryover files from a dirty checkout stay put, `worktree end` refuses to remove a worktree whose HEAD isn't on origin without `--force`, `pr` only ever moves a PR *toward* draft rather than silently flipping an existing draft to ready, and `clean-scope` skips generated and vendored paths and never offers a load-bearing lint directive (`biome-ignore`, `@ts-expect-error`, `noqa`, a shebang) as a comment to clean. Judgment stays with the agent — `clean-scope` is deliberately half a command, answering only the mechanical "which comment lines did this branch add or modify" and leaving the verdict on each one to the caller.
