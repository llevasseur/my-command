...
| `task-bootstrap` | One-time per repo: interview the stack and generate that repo's own `scripts/bootstrap-worktree.sh` so `task` can bootstrap fresh worktrees. A Jira leg asks **only when it sees signal** and writes the contract `ticket` reads. |
| `ticket` | Create and move Jira work items from **the repo's own Jira contract** — `create`, `move`, `link`, `show`, and a thin default that infers the move from branch and PR state. Hardcodes no site, project, issue type, or transition id; resolves the cloud id at run time. Creation is the one thing that waits for a go; transitions fire on their own, but only after the live workflow is re-checked against the contract, and never one on the contract's `never` list. |
...
| `wayfinder` | Run a named campaign of related work as **markdown plans in the repo**, with no issue tracker — cut one base branch, keep one map of active tasks, hand each ticket to `task` off that base, and append a summary to the map as each lands. Only the planning PR at start and the campaign PR at close leave that base — they target the campaign's **integration branch**, which `--integration` names at start and the map records thereafter, defaulting to the repo's default branch. |
...
| `warm` | Ask the claude proxy to hold this session's prompt cache open, so returning after a break reuses a warm cache instead of rebuilding one. One request, no arguments, no state, no teardown — and it reports the registration as **pending**, never as warm, because the proxy arms it only when a real request matches and drops it after 2 minutes if none does. |
| `my-command-tools judge` | **A toolkit verb, not a slash command** — the one that prints a judgement rather than a fact. Asks one of five versioned question sets about one state file and prints the answers as JSON. Gated twice and off by default, wired into no command, and acting on nothing; `--dry-run` prints the exact body and the destination host before anything leaves the device. The eval that would have justified promoting any of it **returned no** — zero subjects carry a number. See [judge-verb](docs/features/judge-verb.md). |
...
| `fb` | `/fb -t feat/checkout-redesign --no-implement the CTA renders in brand blue` | `--no-implement` — implement nothing; `/task` skips its Step 2 and verifies, cleans and PRs what the branch already carries. The text becomes what the verifier should prove, and is optional. Works on the current branch too. |
...
| `wayfinder` | `/wayfinder start the auth revamp` | Cut `wayfinder/auth-revamp` from the campaign's **integration branch** — the default branch unless `--integration` names another — write the map at `<plans>/wayfinder-auth-revamp.md` with its kickoff prompt and first plans, and open the **planning PR** against that same branch so the tickets land before any ticket branch is cut. |
| `wayfinder` | `/wayfinder --integration release/2.0 start the auth revamp` | `--integration <branch>` names the campaign's integration branch: what `wayfinder/<slug>` is cut from and what the **planning** and **campaign** PRs target. Read at `start` only and written into the map's header, so every later operation reads it from there instead of re-deriving it. Absent the flag it is the repo default from `state`; nothing hardcodes `main`. **Not `--base`** — that one is forwarded to the ticket runner and means a *ticket's* cut point. Ticket PRs still target `wayfinder/<slug>`. |
| `wayfinder` | `/wayfinder add task migrate the session store` | Write one plan beside the map and add its row to Active tasks with status `todo` — the only status this operation writes. No branch, no PR. |
| `wayfinder` | *(map Status column)* | Six flat statuses, listed in a legend in every map so the vocabulary travels with the campaign: `todo` (never started — pick it up), `in-progress` (a run has it — leave it), `paused` (deliberately stopped, resumable as-is — pick it back up), `blocked-limit` (the usage window ran out mid-run, **nothing wrong with the work** — resume once it resets), `rejected` (a human turned it down — **never retry it**; it needs a new human decision or a rewritten plan), `redo` (landed but must be done again differently — restart from the plan). The three "stopped" ones are spelled out against each other rather than inferred from their names. A short **Note** column carries the reason, required for `blocked-limit`, `rejected`, and `redo`. |
| `wayfinder` | `/wayfinder execute the next task` | Picks up a task whose status is `todo`, `paused`, `blocked-limit` (once the window reset) or `redo`, never `rejected`; marks it `in-progress`, runs `/task --base wayfinder/<slug>` against the plan's criteria, then **retargets** the resulting PR to the campaign base. A ticket that stops short gets the status saying why — `blocked-limit`, `paused`, or `rejected` — instead of being left on `in-progress`. `--here` / `-h`, `--base <branch>`, `--draft` / `-d`, `--add` / `-a` apply **only here** and forward to the ticket runner verbatim; the charting operations ignore them. |
| `wayfinder` | `/wayfinder complete task 02` | After that ticket's PR merges into the base: **mark its plan done in place** (`**Status:** done · date` — never deleted, moved, or archived), append what was *actually built* to the map's Completed log, and drop its Active tasks row — a completed task carries no status, which is why there is no `done`. The plan's marker describes the *file*, which is kept; the column described a *row*, which is gone. Re-opening one restores its row as `redo` and flips its plan back to `active`. |
| `wayfinder` | *(the plan files)* | Every plan stays in the plans directory for the campaign's whole life, marked done as its task lands, so a task can be restarted from what was **asked** rather than only from the Completed log's prose about what was built. Nothing is copied to an `archive/` or `done/` directory — a second copy is a second source of truth that immediately drifts. |
| `wayfinder` | *(the final ticket)* | `<slug>-zz-retire-done-plans` — a real ticket, created at `start` with the rest, always sorting last, executed after every other task, and deleting every plan including its own. `close` expects it and will not sweep the plans in its place. **Critical, not optional bookkeeping:** it is the only thing that removes a plan, so skipping it leaves the campaign's scaffolding in the repo permanently. |
| `wayfinder` | `/wayfinder close` | One **campaign PR** from the base branch to the integration branch the map records; after it merges, retire the map and delete the base branch. The plans went with the `zz` ticket. Merges nothing itself. |
| `wayfinder` | `/wayfinder --unattended execute the next task` | Same operations, no human in the loop: tickets run through `/god --base wayfinder/<slug> --into wayfinder/<slug>` instead of `/task` — the campaign base named twice, as cut point and as merge target, since absent `--into` a ticket would merge into the default branch — and the planning, ticket, and campaign merges are authorised. **Must be typed on the invocation that acts** — no operation infers it from an invoking command, from an earlier operation, or from the map — because a campaign multiplies whatever it authorises. Without it, every PR is left open for review, unchanged. |
| `wayfinder` | *(the map's `**Unattended:**` line)* | `start` records the campaign's mode from whether `--unattended` was typed there, and the map's **agent kickoff prompt is generated against it** — the one path that puts the flag in front of the next agent to type. The prompt *is* the resume path, so a fixed "stop after opening the PR" line meant every resume silently downgraded an unattended campaign to the reviewed default and it never finished. On `yes` the prompt says to type `--unattended` and carry the ticket through to merged; on `no` it is today's wording, which is also how every earlier map reads. The map still authorises nothing — the flag is read only off the invocation. Risk accepted in [ADR 0006](docs/adrs/0006-unattended-campaigns-resume-unattended.md): a map is a file, so whoever can edit it can put a merge in a later resume's hands. |
| `dev` | `/dev <complex idea>` | The whole pipeline, unattended: grill the idea against `docs/specs/` and `docs/adrs/` (12 rounds, one question at a time, **one** griller kept alive across rounds), write an ADR for every decision it made itself, `/wayfinder start --integration <integration branch>` → planning PR merged with `/god --into <integration branch>`, tickets dispatched through `/manage` as `/wayfinder --unattended execute NN` in **waves bounded by file-scope lanes and `/manage`'s cap of 8**, each landing via `/god --base wayfinder/<slug> --into wayfinder/<slug>`, then `/wayfinder close` and one final `/god --into <integration branch>`. |
| `dev` | `/dev --into release/2.0 <idea>` | `--into` / `-i <branch>` — the campaign's **integration branch**: what `wayfinder/<slug>` is cut from, and what the planning PR and the campaign PR merge into. Absent it, that is the repo's default branch from `my-command-tools state`, exactly as before; neither path hardcodes a branch name. Resolved once and then passed to `/wayfinder start --integration <branch>` and to both `/god` merges. **Not a cut point** — each ticket still runs `/god --base wayfinder/<slug> --into wayfinder/<slug>`, so this moves the campaign and never a ticket. |
...
| `cp` | `/cp task add a dark-mode toggle to settings` | Default — shape the prompt to stand alone for an agent that can't see this conversation, copy `/task add a dark-mode toggle to settings` to the clipboard, and reply `Done! 0` — the slot the copy landed in, so `/cp --again 0` puts it back. The named command is never run, loaded, or printed. |
| `cp` | `/cp -v review 42` | `--verbatim` / `-v` — copy the prompt exactly as typed, no shaping. |
| `cp` | `/cp --again` | `--again` / `-a` — restore the last copy from `~/.claude/cp-last.txt` to the clipboard. No composing, no reading, no tokens spent on the text. A trailing slot number reaches an older ring entry (`/cp --again 2` → `cp-last.2.txt`, five deep), and the reply names the slot it restored. |
...
| `ticket` | `/ticket ENG-412` | Default — the thin form. Reads the contract, then infers the one move that is due from the branch and the PR state: `start` when work has begun, `review` when an open non-draft PR carries the key. Already in the target status is a no-op; a branch and PR that disagree is a stop, not a guess. |
| `ticket` | `/ticket create story add a dark-mode toggle to settings` | `create [story\|bug\|task\|chore\|spike] <summary>` — the word is the **Jira issue type**; omitted, the contract's default is used. The template is picked by whether the change is **user-visible**, not by that issue type, and pre-filled from the diff. Renders the whole ticket and **waits for an explicit go** — creation is the only irreversible action. |
| `ticket` | `/ticket -y create fix the retry backoff` | `--yes` / `-y` skips the approval **and** the template question together, inferring the template from the diff. `/god` passes it so unattended runs never block. It never overrides the contract's `never` list. `--template <story\|bug\|chore\|spike\|technical>` forces the template instead. |
| `ticket` | `/ticket move ENG-412 start` | `move <KEY> start\|review` — fires that lifecycle transition without asking, after re-reading the **live** transitions and confirming the contract's declared id **and** name still match. A mismatch stops the run and reports a workflow change rather than firing the closest match. |
| `ticket` | `/ticket link ENG-412 blocks ENG-418` | `link <KEY> blocks\|blocked-by\|relates <KEY>` — always explicit; nothing else in the command infers a link. `/manage` passes the dependency graph it already builds, so stacked units become `Blocks` links. `show <KEY>` reads one item and writes nothing. |
| `task` | `/task -a ticket adopt ENG-412 for this work add the retry backoff` | `--add` / `-a ticket` — in a task run `/ticket` **only adopts an existing key**: it fires `start` and attaches the PR as a remote link, and **never creates and never fires `review`**, because several task runs can feed one ticket and calling development complete is your call. |
...
| `my-command-tools judge` | `my-command-tools judge --set clean-comment --state-file state.json -n` | `--dry-run` / `-n` — print the exact request body and the host it would go to, and make no network call. Ungated on purpose: reading what would be sent must not require opting into sending it. |
| `my-command-tools judge` | `my-command-tools judge --set trim --state-file state.json --judge` | `--judge` — opt this one invocation in. `TYPESAFE_API_KEY` must be set as well; neither gate alone is enough, and with no key nothing is sent and nothing is said. `--set` takes `trim`, `clean-comment`, `dispatch-route`, `verify-regression` or `bash-shape`. |
| `my-command-tools judge` | `my-command-tools judge --set trim --state-file s.json --judge --shadow --baseline b.json` | `--shadow` — record what the layer said beside what `--baseline <path>` says the existing path decided, and act on neither. Records land under `~/.my-command/judge/<set>/`, outside any checkout, so one can never be committed by accident. |
| `warm` | `/warm` | The only form — no flags, no arguments. POSTs `{"sessionId", "hours": 8}` to `http://127.0.0.1:${CLAUDE_PROXY_PORT:-8787}/__warm` and reports the three facts the proxy's answer supports: the registration is **pending**, it arms on the next matching request from this session, and it expires in 2 minutes if none arrives. "Session kept warm" is forbidden wording. A refused connection is reported as "the proxy is not running" and **never retried**. |
| `task` | `/task --add warm keep this session warm while I'm away` | `--add` / `-a` — the composition point. `/warm` needs no arguments and no teardown, so it fires once at the start of the run and the run carries on whether or not the proxy answered. |
...
2. **Personal commands** — bare commands (`/task`) copied into
   `~/.claude/commands`.
3. **Skills** — workflows such as `$task` copied as complete skill folders into
   `~/.agents/skills/<name>/`, which serves **both Codex and opencode**. Set
