...
| `ideate` | `/ideate` | Default — survey the repo through all four evidence sources, compose at most 3 ranked proposals, write them all to the ledger as `proposed`, then ask **once** which to accept. Never implements, never opens a branch or PR. |
...
| `ideate` | `/ideate -n` | `--dry-run` / `-n` — report the proposals and write **nothing at all**, not even the `proposed` rows, and skip the sign-off question. Still reads every ledger tier for dedupe. |
