...
| `manage` | `/manage -n <goal>` | `--dry-run` / `-n` — print the routing plan (units, waves, dependencies, branch names, each unit's base, the forwarded flags it carries and the ones dropped for it, delegate) and spawn nothing. No task list, no subagent, no branch. |
...
| `wayfinder` | `/wayfinder --unattended execute the next task` | Same operations, no human in the loop: tickets run through `/god` instead of `/task`, and the planning, ticket, and campaign merges are authorised. **Must be typed on the invocation that acts and is never inherited** — not from the map, its kickoff prompt, an invoking command, or an earlier operation — because a campaign multiplies whatever it authorises. Without it, every PR is left open for review, unchanged. |
