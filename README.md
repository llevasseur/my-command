...
| `wayfinder` | `/wayfinder complete task 02` | After that ticket's PR merges into the base: delete its plan, append what was *actually built* to the map's Completed log, and drop its Active tasks row — a completed task carries no status, which is why there is no `done`. Re-opening one restores its row as `redo`. |
| `wayfinder` | `/wayfinder close` | One **campaign PR** from the base branch to the integration branch the map records; after it merges, retire the map and every plan and delete the base branch. Merges nothing itself. |
