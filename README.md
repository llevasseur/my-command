...
| `wayfinder` | `/wayfinder execute the next task` | Runs `/task --base wayfinder/<slug>` against the plan's criteria, then **retargets** the resulting PR to the campaign base. `--here` / `-h`, `--base <branch>`, `--draft` / `-d`, `--add` / `-a` apply **only here** and forward to `/task` verbatim; the charting operations ignore them. |
